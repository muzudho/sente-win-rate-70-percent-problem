...
if __name__ == '__main__':
    ...
    try:
        ...
        actual_black_rate = (expected_black_target + expected_white_target - 1) / expected_max_bout_count
        print(f"[{datetime.datetime.now()}] {actual_black_rate=} ＝ {expected_white_target=} ／ {expected_max_bout_count=}")
        print(f"[{datetime.datetime.now()}] {(expected_black_win_rate * actual_black_rate)=} ＝ {expected_black_win_rate=} × {actual_black_rate=}")
        print(f"[{datetime.datetime.now()}] {(expected_black_win_rate / actual_black_rate)=} ＝ {expected_black_win_rate=} ／ {actual_black_rate=}")
        print(f"[{datetime.datetime.now()}] {(actual_black_rate / expected_black_win_rate)=} ＝ {actual_black_rate=} ／ {expected_black_win_rate=}")
