...
def write_coin_toss_log(output_file_path, black_win_rate, b_point, w_point, round_total, black_wons):
    ...
    with open(output_file_path, 'a', encoding='utf8') as f:
        ...
        text = f"[{datetime.datetime.now()}]  先手勝率 {black_win_rate*100:2.0f} ％ --調整後--> 先手が勝った確率{black_won_rate*100:8.4f} ％（± {error*100:7.4f}）  {max_number_of_bout_in_freeze_turn:2}本勝負（ただし、{b_point:>3}本先取制。後手は最初から{w_advantage:2}本もつアドバンテージ）  先手勝ち数{black_wons:7}／{round_total:7}対局試行"
        print(text) # 表示
        f.write(f"{text}\n")    # ファイルへ出力
