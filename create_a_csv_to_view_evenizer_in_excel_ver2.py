...
def automatic(specified_failure_rate, specified_turn_system, specified_trials_series):
    ...
    #
    csv_file_path_of_view = get_even_view_csv_file_path(spec=spec, trials_series=specified_trials_series)
    ...
    def on_each(best_record):
        ...
        # 対象外のものはスキップ　［将棋の引分け率］
        if specified_failure_rate != best_record.failure_rate:
            return

        # 対象外のものはスキップ　［先後の決め方］
        if specified_turn_system != Converter.code_to_turn_system(best_record.turn_system_str):
            return

        # # 対象外のものはスキップ　［試行シリーズ数］
        # if specified_trials_series != best_record.trials_series:
        #     return
