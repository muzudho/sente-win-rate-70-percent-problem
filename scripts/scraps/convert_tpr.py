        ...
        for basename in basename_list:
            ...
            try:
                ...
                if True: #if 't_step_divisible_by_h_step' not in df.columns.values:
                    df['t_step_divisible_by_h_step'] = df[['t_step', 'h_step', 'h_time']].apply(lambda X:SeriesRule.let_t_step_divisible_by_h_step(t_step=X['t_step'], h_step=X['h_step'], h_time=X['h_time']), axis=1)
                    dirty = True
