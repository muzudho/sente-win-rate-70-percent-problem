...
def create_all_files(upper_limit_span):
    ...
    for failure_rate_percent in range(0, int(UPPER_LIMIT_FAILURE_RATE * 100) + 1, 5): # 5％刻み。 100%は除く。0除算が発生するので
        ...
        print(f"[{datetime.datetime.now()}] in create_all_files {failure_rate=:.2f}")
        ...
        for p_percent in range(50, 96):
            ...
            for turn_system in [ALTERNATING_TURN, FROZEN_TURN]:
                ...
                if is_new:
                    ...
                    # ［目標の点数］
                    for span in range(1, upper_limit_span):
                        print(f"[{datetime.datetime.now()}] CREATE_FILE {span=}")

                        # ［後手で勝ったときの勝ち点］
                        for t_step in range(1, span + 1):
                            #print(f"[{datetime.datetime.now()}] CREATE_FILE {t_step=}")

                            # ［先手で勝ったときの勝ち点］
                            for h_step in range(1, t_step + 1):
                                #print(f"[{datetime.datetime.now()}] CREATE_FILE {h_step=}")
                    ...
                    print(f"[{datetime.datetime.now()}][turn_system={turn_system_str:11}  p={p:.2f}  failure_rate={spec.failure_rate:.2f}] CREATE_FILE  write file to `{csv_file_path_to_wrote}` ...")
...
if __name__ == '__main__':
    ...
    try:
        # まず、ファイルを全部作る
        create_all_files(
            upper_limit_span=4  # 11     # 数字が増えると処理が重くなる
