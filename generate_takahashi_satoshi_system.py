...
from library import round_letro, calculate_probability, let_points_from_require
    ...
    try:
        ...
        for p in df['p']:
                ...
                #for w_require in range (1, 2): # 後手に必要な先取本数を 1 に固定する場合
                    ...
                    if error < best_error:
                        ...
                        if best_error != OUT_OF_ERROR:
                            ...
                            #         message = f"[▲！先手勝率が［７６％～７８％）（{p}）なら、７本勝負を超えるケース（{max_number_of_bout_in_freeze_turn} 黒{b_require} 白{w_require}）は、調整を諦めます]"
                            #         print(message)
                            #         process_list.append(f"{message}\n")
                            #         continue
            ...
            with open(LOG_FILE_PATH, 'a', encoding='utf8') as f:
                ...
                #
                max_number_of_bout_in_freeze_turn = best_b_require + best_w_require - 1
                ...
                text += f"先手勝率 {p*100:2.0f} ％ --調整後--> {best_balanced_black_win_rate*100:6.4f} ％ （± {best_error*100:>7.4f}）  対局数ｍ～{max_number_of_bout_in_freeze_turn:>2}  先手勝ち{b_point:2.0f}点、後手勝ち{w_point:2.0f}点の{target_point:3.0f}点先取制"
